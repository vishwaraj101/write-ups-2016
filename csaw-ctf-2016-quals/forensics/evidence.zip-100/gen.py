...
def generate_file():
    ...
    return ''.join([secrets.choice(string.printable) for _ in xrange(secrets.SystemRandom().randint(100, 1000))])

def generate_filename():
    return ''.join(secrets.choice(string.lowercase + string.digits) for _ in range(10))
...
def zip(dst):
    ...
    for x in range(len(flag)/4):
        ...
        zf.writestr(arcname, content, compress_type=secrets.choice([0,8]))
    curr_flag_pos = 0
    for info in zf.infolist():
        ...
        info.create_version ^= secrets.SystemRandom().randint(0,0xff)
        info.create_system ^= secrets.SystemRandom().randint(0,0x0f)
        info.extract_version ^= secrets.SystemRandom().randint(0,0xff)
        info.compress_type ^= secrets.SystemRandom().randint(0x1,0xff)
